...
class AbstractEnv(gym.Env):
    ...
    def _info(self, obs: Observation, action: Action) -> dict:
        ...
        ego_vehicles = self.controlled_vehicles
        for action_per_vehicle, vehicle in zip(action, ego_vehicles):
            info_per_vehicle = {
                "speed": vehicle.speed,
                "crashed": vehicle.crashed,
                "action": action_per_vehicle,
            }
            try:
                info_per_vehicle["cost"] = self._cost(action_per_vehicle)
            except NotImplementedError:
                pass

            info[vehicle] = info_per_vehicle
        info["eps"] = self.steps
        return info
